...
IMG_OBSTACLE = pygame.image.load("image/asset_obstacle.png")
IMG_STARTFIN = pygame.image.load("image/asset_startfin.png")
...
IMG_UNDO_DISABLED = pygame.image.load("image/btn_undo_disable.png")
IMG_REDO_DISABLED = pygame.image.load("image/btn_redo_disable.png")
...
IMG_UNDO_DISABLED = pygame.transform.scale(IMG_UNDO_DISABLED, (UNDO_W, UNDO_H))
IMG_REDO_DISABLED = pygame.transform.scale(IMG_REDO_DISABLED, (UNDO_W, UNDO_H))

COLOR_BOX = (255, 255, 255)
COLOR_ACTIVE = (180, 220, 255)
COLOR_TEXT = (0, 0, 0)

COLOR_GRID = pygame.Color("#003D04")
COLOR_EMPTY = pygame.Color("#007108")
COLOR_LINE = pygame.Color("#009DBD")
...
def start_menu():
    ...
    create_button_rect = pygame.Rect(center_x - 55, center_y + 130, 200, 60)
...
def draw_grid(rows, cols, cs, grid, ox, oy):
    ...
    stone = pygame.transform.scale(IMG_OBSTACLE, (cs, cs))

    dots = pygame.transform.scale(IMG_STARTFIN, (cs, cs))
...
def draw_path(path, cs, ox, oy):
    ...
    pygame.draw.lines(SCREEN, COLOR_LINE, False, points, 10)
...
def main_editor(rows, cols):
    ...
    buttons = {
        ...
        "set_sf": pygame.Rect(BTN_X, 220, BTN_W, BTN_H),
        "set_obs": pygame.Rect(BTN_X, 280, BTN_W, BTN_H),
    ...
    while True:
        ...
        if undo:
            ...
            SCREEN.blit(IMG_UNDO_DISABLED, buttons["undo"].topleft)

        if redo:
            SCREEN.blit(IMG_REDO, buttons["redo"].topleft)
        else:
            SCREEN.blit(IMG_REDO_DISABLED, buttons["redo"].topleft)

        txt = FONT.render(f"Obstacles: {obs_count}", True, (255, 255, 255))
        SCREEN.blit(txt, (WIDTH - 320, 320))
